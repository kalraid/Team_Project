...
game.makeHoles = function(labels, x0ffset, y0ffset){
   game.holes = [];
   for(var i = 0; i < labels.length; i++){
      var newHole = Object.create(game.hole);
      newHole.holeLocation = [x0ffset + game.hole.spacing*i, y0ffset];
...
game.mole = {
      ...
      drawEyes: function(xPosition, yPosition){
         ...
         atom.context.arc(xPosition + this.eye0ffset, yPosition - this.eye0ffset, this.eyeSize, 0, Math.PI*2);
         atom.context.fill();
         atom.context.beginPath();
         atom.context.fillStyle = this.eyeColor;
         atom.context.arc(xPosition - this.eye0ffset, yPosition - this.eye0ffset, this.eyeSize, 0, Math.PI*2);
      ...
      drawWhiskers: function(xPosition, yPosition){
         ...
         atom.context.moveTo(xPosition - 30, yPosition);
         atom.context.moveTo(xPosition + 10, yPosition);
         atom.context.moveTo(xPosition + 30, yPosition);
         atom.context.moveTo(xPosition - 10, yPosition + 5);
         atom.context.moveTo(xPosition - 30, yPosition + 10);
         atom.context.moveTo(xPosition + 10, yPosition + 5);
         atom.context.moveTo(xPosition + 30, yPosition + 10);
         atom.context.stroke();
      }
}
game.drawHoles = function(holeLabels, x0ffset, y0ffset){
   for(i = 0; i < holeLabels.length; i++){
      atom.context.fillStyle = game.hole.color;
      var holeLocation = [x0ffset + game.hole.spacing*i, y0ffset];
...
game.hole = {
      ...
      draw: function(){
         ...
         this.drawLabel();
         if(this.active === true){
            this.drawMole(this.holeLocation[0], this.holeLocation[1] - this.mole0ffset);
         };
      },
      drawHole: function(){
         atom.context.fillStyle = this.color;
      ...
      drawLabel: function(){
         ...
         atom.context.fillText(this.label, this.holeLocation[0] - this.size, this.holeLocation[1] + this.label0ffset);
      },
...
game.drawBackground = function(){
   atom.context.beginPath();
   atom.context.fillStyle = 'skyblue';
